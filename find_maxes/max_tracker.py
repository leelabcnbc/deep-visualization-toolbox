...
class NetMaxTracker(object):
    def __init__(self, layers=None, is_conv=None, n_top=10, initial_val=-1e99):
        if layers is None:
            layers = deepcopy(default_layers)
        if is_conv is None:
            is_conv = deepcopy(default_is_conv)
        self.layers = layers
        self.is_conv = is_conv
