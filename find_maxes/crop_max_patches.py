...
def main():
    ...
    parser.add_argument('outdir', type=str,
                        help=r'Which output directory to use. Files are output into outdir/layer/unit_%%04d/{maxes,deconv,backprop}_%%03d.png')
    parser.add_argument('layer', type=str, help='Which layer to output')
    parser.add_argument('net_info', type=str, help='info.json for net')
    ...
    # load the info
    with open(args.net_info, 'rt') as f_info:
        layer_info = json.load(f_info, encoding='utf-8')['layer_info']
    layer_info_spec = [layer_info[x] for x in layer_info]
    ...
    with WithTimer('Saved %d images per unit for %s units %d:%d.' % (args.N, args.layer, args.idx_begin, args.idx_end)):
        output_max_patches(mt, net, args.layer, args.idx_begin, args.idx_end,
                           ...
                           (args.do_maxes, args.do_deconv, args.do_deconv_norm, args.do_backprop, args.do_backprop_norm,
                            args.do_info),
                           layer_info_spec)
